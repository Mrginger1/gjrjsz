...
<script type="text/javascript">
	function selok() {
		opener.document.getElementById("fileurl").value = myform.p.value;
		window.close();
	}
</script>
</head>
<body>
	<div style="margin: 15px;">
		...
		<form id="addForm" class="layui-form" action="upload/files.action" name="myform" method="post"
			...
			<c:if test="${imageFileName ne null }">
				<div class="layui-form-item">
					...
					<div class="layui-input-inline shortInput">
						...
						<button class="layui-btn" type="button" onclick="selok();">保存文件</button>
					</div>
				</div>
			</c:if>
			<c:if test="${imageFileName eq null }">
				<div class="layui-form-item">
					<div class="layui-input-block">
						<button class="layui-btn" type="submit" id="sub">上传文件</button>
					</div>
				</div>
			</c:if>
		</form>
